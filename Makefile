...
run: tailwind ## **Run** the server
	@go run cmd/*.go

run_dev: ## **Run** the server in _watch mode_. The watchmode will load templates during request time.
	ENV=DEV go run cmd/*.go &
	make tailwind_dev

tailwind: ## **Run** the tailwind cli. This step will generate the css files for the web app
	tailwindcss -i ./styles/styles.css -o ./assets/styles.css -m


tailwind_dev: ## **Run** the tailwind in _watch mode_.
	tailwindcss -i ./styles/styles.css -o ./assets/styles.css -m --watch
